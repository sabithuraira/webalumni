...
<nav class="bg-[#343C6A]">
    <div class="mx-auto flex justify-between items-center py-2">
        ...
        <div class="flex items-center space-x-4 md:space-x-10 mr-10">
            ...
            @if (Auth::check())
                <a href="{{ url('dashboard') }}" class="text-white font-bold border-white border-2 rounded-full text-[15px] px-3 py-2">
                    ADMIN
                </a>
            @else 
                <button id="loginButton" class="text-white font-bold border-white border-2 rounded-full text-[15px] px-3 py-2">
                    LOGIN
                </button>
            @endif
        </div>
